...
int Erase_page(uint32_t start, uint32_t len)
{
	  ...
	  EraseInitStruct.TypeErase	= FLASH_TYPEERASE_PAGES;	//É¾³ı·½Ê½
	  EraseInitStruct.Page		  = GetPages(start);					  //³¬Ê¼Ò³ºÅ
	  EraseInitStruct.NbPages	  = len;					  //Ò³µÄÊıÁ¿
	  EraseInitStruct.Banks 	  = FLASH_BANK_2;					  //bankºÅ
	  
	  HAL_FLASH_Unlock();		  //½âËø£¬ÒÔ×¼±¸½øĞĞFLASH²Ù×÷
	  status=HAL_FLASHEx_Erase(&EraseInitStruct, &PageError);	  //²Á³ı
	  HAL_FLASH_Lock();
      HAL_FLASH_Lock();
    return status;
}

/** @bieaf 	Ğ´É´?Ù¶Ë½?,STM32L4xxxÖ»?Ë«??Ğ´É«
	* @param 	addr     		Ğ´É«??Ö·
	* @param 	buff       	Ğ´É«Ë½??Ç°Ê¼?Ö·
	* @param 	word_size  	Ó¤?
	* @return	none				×µ?Öµ
 */
...
uint32_t uint8Touint32(uint8_t *buf)
{
