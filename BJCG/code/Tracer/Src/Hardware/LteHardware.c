...
void LtePowerManage(unsigned char moduleType,unsigned char powerCtrl)
{
    ...
    else if((moduleType == LTE_NBIOT))
    {
        if(powerCtrl == ON)
        {
					 HAL_GPIO_WritePin(GPIOB,EN_3V8_Pin, GPIO_PIN_RESET);//lte电源使能
            HAL_GPIO_WritePin(GPIOB,EN_3V8_Pin, GPIO_PIN_SET);//lte电源使能
...
{
   uint8_t Res;
    gps_res =UART3_ReceiveByte();//(USART1->DR);	//读取接收到的数据
    ...
    if(USART_RX_BUF[0] == '$' && USART_RX_BUF[2] == 'N'&& USART_RX_BUF[3] == 'G'&& USART_RX_BUF[4] == 'G' && USART_RX_BUF[5] == 'A')			//确定是否收到"GPRMC/GNRMC"这一帧数据
    {
      if(gps_res == '\n')									   
      {
    ...
    for (i = 0 ; i <= 9 ; i++)
    {
      ...
      else
      {
        ...
        if ((subStringNext = strstr(subString, ",")) != NULL)
        {
		  ...
		  case 5: memcpy(Save_Data.E_W, subString, subStringNext - subString);break;	//获取E/Wbreak;	//获取经度信息
          case 6:memcpy(usefullBuffer, subString, subStringNext - subString);break;	//获取UTC时间
          case 9:memcpy(Save_Data.altitude, subString, subStringNext - subString);break;	//获取UTC时间
