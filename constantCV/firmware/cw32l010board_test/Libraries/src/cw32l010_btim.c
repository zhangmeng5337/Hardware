...
void BTIM_TimeBaseInit(BTIM_TypeDef* BTIMx, BTIM_TimeBaseInitTypeDef* BTIM_TimeBaseInitStruct)
{
    ...
    /* 使能BTIM的APB时钟 */
    CW_SYSCTRL->APBEN2 |= SYSCTRL_APBEN2_BTIM123_Msk;
    
    /* 配置BTIM的工作模式 */
    BTIMx->ARR = BTIM_TimeBaseInitStruct->BTIM_Period;
    BTIMx->PSC = BTIM_TimeBaseInitStruct->BTIM_Prescaler - 1;
...
void BTIM_ITConfig(BTIM_TypeDef* BTIMx, uint16_t BTIM_IT, FunctionalState NewState)
{
    ...
    NewState ? ( BTIMx->DIER |= BTIM_IT) : (BTIMx->DIER &= ~BTIM_IT);
}
/*******************************************************************************
...
void BTIM_ETRConfig(BTIM_TypeDef* BTIMx, BTIM_ETRConfigTypeDef *BTIM_ETRConfigStruct)
{
    ...
    REGBITS_MODIFY(BTIMx->SMCR, BTIMx_SMCR_ETRFLT_Msk, BTIM_ETRConfigStruct->BTIM_ETR_Fliter);
}
...
void BTIM_DMACmd(BTIM_TypeDef* BTIMx, uint16_t BTIM_DMASource, FunctionalState NewState)
{
    ...
    NewState ? (BTIMx->DIER |= BTIM_DMASource) : (BTIMx->DIER &= ~BTIM_DMASource);
}
