...
int32_t main(void)
{
 ...
 /**************************主机 <-> 从机**********************/
    ...
    //拉低CS，选中SPI从机
    SPI_CS_LOW();
		SPI_ITConfig(CW_SPI,SPI_IT_TXE, ENABLE);
		while(TxCounter < BufferSize);
  
    //发送完成
    while(SPI_GetFlagStatus(CW_SPI,SPI_FLAG_BUSY) == RESET);
    ...
    #ifdef SPI_SLAVE
    SPI_ITConfig(CW_SPI,SPI_IT_RXNE, ENABLE);
...
{
			#ifdef SPI_MASTER
  if(SPI_GetITStatus(CW_SPI,SPI_IT_TXE) != RESET)
  {
    if(TxCounter == BufferSize-1)
    {
      SPI_ITConfig(CW_SPI,SPI_IT_TXE, DISABLE);
    }
  ...
  #ifdef SPI_SLAVE
  if(SPI_GetITStatus(CW_SPI,SPI_IT_RXNE) != RESET)
  {
