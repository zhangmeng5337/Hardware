...
uint8_t u8SendLen =16;
uint32_t u32RecvLen = 16;
...
int32_t main(void)
{
    ...
    while (1)
    {
		I2C_SlaveRecDataFromMaster(CW_I2C1,u8Recdata,&u32RecvLen);//等主机写数据,MCU作为从机接收主机发送的数据
        I2C_SlaveSendDataToMaster(CW_I2C1,u8Recdata,u8SendLen);//等待主机寻址，MCU作为从机给发送数据
    }
}
