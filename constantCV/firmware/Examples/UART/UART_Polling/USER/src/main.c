...
uint8_t UART_RecvBuf_Polling(UART_TypeDef *UARTx, uint8_t *RxBuf)
{
    ...
    {
        //等待RC
        while (UART_GetFlagStatus(UARTx, UART_FLAG_RC) == RESET);
        //清RC
        UART_ClearFlag(UARTx, UART_FLAG_RC);
        //ERROR: PE or FE
        if (UART_GetFlagStatus(UARTx, UART_FLAG_PE | UART_FLAG_FE))
        {
