...
int32_t main(void)
{
    ...
    printf("\r\nCW32F010 UART AutoBaudRate\r\n");
	
	printf("\r\nPC发送以0xF8作为数据头的数据流\r\n");

    //自动波特率检测
    AutoBaudRate(DEBUG_UARTx, CW_GTIM1);
    while (1)
    {
        //中断收发
    }
...
void AutoBaudRate(UART_TypeDef *USARTx, GTIM_TypeDef *GTIMx)
{
    ...
    USARTx->CR1_f.RXEN = 0;
    //使能RXD上升沿中断
    CW_GPIOB->RISEIE_f.PIN6 = 1;
    //开启GTIM
    GTIMx->CR1_f.CEN = 1;
    while (CW_GPIOB->ISR_f.PIN6 == 0);
    //失能RXD上升沿中断
    CW_GPIOB->RISEIE_f.PIN6 = 0;
    //关闭GTIM
    GTIMx->CR1_f.CEN = 0;
    //清除中断标志
	CW_GPIOB->ICR_f.PIN6 = 0;
    //计算BRRI、BRRF
    temp = ((GTIMx->CNT) + 0) / 4;
...
void GTIM_Configuration(void)
{
	__SYSCTRL_GTIM1_CLK_ENABLE();//使能GTIM1的时钟
	
	CW_GTIM1->TISEL_f.TI1SEL = 4;//TI1通道输入捕获信号来源选择UART2_RXD
	CW_GTIM1->CR2_f.TI1S = 0;//GTIM_CH1引脚连接到TI1输入
	CW_GTIM1->CCMR1CAP_f.IC1F = 0;//无滤波
	CW_GTIM1->CCER_f.CC1NP = 0;
	CW_GTIM1->CCER_f.CC1P = 1;//极性翻转，低电平有效
	CW_GTIM1->SMCR_f.SMS = 5;//门控模式
	CW_GTIM1->SMCR_f.TSH = 0;
	CW_GTIM1->SMCR_f.TS = 5;//TRGI 信号的来源为 TI1FP1
	CW_GTIM1->ARR_f.ARR = 0xFFFF;
}
