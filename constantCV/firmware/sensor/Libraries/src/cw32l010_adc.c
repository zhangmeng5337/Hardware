...
void ADC_Init(ADC_InitTypeDef *ADC_InitStruct)
{
    ...
    REGBITS_SET(CW_SYSCTRL->APBEN1, SYSCTRL_APBEN1_ADC_Msk);  //打开ADC时钟

    REGBITS_MODIFY(CW_ADC->CR,
                   ADC_CR_ENS_Msk | ADC_CR_CLKSRC_Msk | ADC_CR_CONT_Msk,
 ...
 *        @arg ENABLE
 */
void ADC_ExtTrig1Cfg(uint16_t ADC_TRIG, FunctionalState NewState)
{
    uint16_t trigMask = 0;
    /* Check the parameters */
    assert_param(IS_FUNCTIONAL_STATE(NewState));
...
{
    assert_param(IS_ADC_SQR_CH(SqrCHx));
    
    uint32_t *pAdcResult = (uint32_t *)&CW_ADC->RESULT0;
    
    return *(pAdcResult+SqrCHx);
