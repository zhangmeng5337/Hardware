...
void GPIO_Init(GPIO_TypeDef  *GPIOx, GPIO_InitTypeDef *GPIO_Init)
{
    ...
    else
    {
        ...
        while ((GPIO_Init->Pins >> Position) != 0U)
        {
            IOcurrent = GPIO_Init->Pins & (1UL << Position);

            if (IOcurrent != 0U)
            {
                if (Position < 8)
                {
                    GPIOx->AFRL = (GPIOx->AFRL & (~(0x0fu << (Position<<2)))) | (GPIO_Init->Alternate << (Position<<2));
                }
                else
                {
                    GPIOx->AFRH = (GPIOx->AFRH & (~(0x0fu << ((Position-8)<<2)))) | (GPIO_Init->Alternate << ((Position-8)<<2));
                }
            }
