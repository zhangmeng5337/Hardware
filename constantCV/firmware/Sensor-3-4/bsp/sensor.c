#include "sensor.h"
#include "cs1237.h"
#include "main.h"
#include "kalman.h"
#include "cw32l010_gtim.h"
#include "reg.h"
#include "key.h"
extern struct cs1237_device g_cs1237_device_st;

adc_stru adc_usr;
adc_stru *getadc(void)
{
    return &adc_usr;
}
unsigned char *getAdcReconfig(void)
{
    return &g_cs1237_device_st.reconfig;
}
void adc_init(void)
{
    //cs1237_init(&g_cs1237_device_st, DEV_FREQUENCY_10, DEV_PGA_1, DEV_CH_A);
    cs1237_init(&g_cs1237_device_st, GetReg()->pb[eREG_ADC_RATE].val_u + 1,
                GetRegPrivate()->pga, DEV_CH_A);


}

void adc_config()
{


    if (getKey()->indat[0].spd == 0)
    {
        GetReg()->pb[eREG_ADC_RATE].val_u = getKey()->indat[0].spd;

    }
    else
        GetReg()->pb[eREG_ADC_RATE].val_u = 1;
    cs1237_init(&g_cs1237_device_st, GetReg()->pb[eREG_ADC_RATE].val_u + 1,
                DEV_PGA_1, DEV_CH_A);
    g_cs1237_device_st.reconfig = 0;
}
unsigned char unit_convert(float dat)
{
    unsigned char i = 0;
    for (i = 0; i < 13; i++)
    {
        if (GetReg()->pb[eREG_UNIT].val_u == i)
            break;
    }
    if (i == 13)
        return 0;



    switch (GetReg()->pb[eREG_UNIT].val_u)
    {
        case epa   :
            adc_usr.data_unit_app = dat;
            break;
        case eKpa  :
            adc_usr.data_unit_app = dat / 1000.0;
            break;
        case eMpa  :
            adc_usr.data_unit_app = dat / 1000000.0;
            break;
        case emmH2O:
            adc_usr.data_unit_app = dat * 0.101972;
            break;
        case emH2O :
            adc_usr.data_unit_app = dat * 0.000101972;
            break;
        case ebar  :
            adc_usr.data_unit_app = dat * 0.00001;
            break;
        case epsi  :
            adc_usr.data_unit_app = dat * 0.000145037744;
            break;
        case eatm  :
            adc_usr.data_unit_app = dat * 0.000009869233;
            break;
        case ekg   :
            adc_usr.data_unit_app = dat * 0.101971621298;
            break;
        case emm   :
            adc_usr.data_unit_app = dat * 0.007500616827;
            break;
        case em    :
            adc_usr.data_unit_app = dat * 0.000007500616827;
            break;
        case eT    :
            adc_usr.data_unit_app = dat;
            break;
        case eF    :
            adc_usr.data_unit_app = dat + 273.15;
            break;
        default    :
            return 0;
            break;

    }
    return 1;
    //filter(5,sensor_pressure.buf);
}
unsigned char factory_unit_convert(float dat)
{
    unsigned char i = 0;
    unsigned char result;
    float tmp;
    result = 1;

    if (GetRegPrivate()->typ == 0)
    {
        switch (GetRegPrivate()->unit)
        {
            case epa:
                tmp = adc_usr.dat_unit_factory * 1;
                result = 1;
                break;
            case eKpa  :
                tmp = adc_usr.dat_unit_factory / 1000.0;
                break;
            case eMpa  :
                tmp = adc_usr.dat_unit_factory / 1000000.0;
                break;
            case emmH2O:
                tmp = adc_usr.dat_unit_factory * 9.806613580198485;
                break;
            case emH2O :
                tmp = adc_usr.dat_unit_factory * 9806.613580198486;
                break;
            case ebar  :
                tmp = adc_usr.dat_unit_factory * 10000;
                break;
            case epsi  :
                tmp = adc_usr.dat_unit_factory * 6894.756995117078;
                break;
            case eatm  :
                tmp = adc_usr.dat_unit_factory * 101324.99658281449;
                break;
            case ekg   :
                tmp = adc_usr.dat_unit_factory * 9.806649999980076;
                break;
            case emm   :
                tmp = adc_usr.dat_unit_factory * 133.3223684217938;
                break;
            case em    :
                tmp = adc_usr.dat_unit_factory * 133322.3684217938;
                break;
            default    :
                result = 0;
                break;

        }
    }
    else if (GetRegPrivate()->typ == 1)
    {
        switch (GetRegPrivate()->unit)
        {
            case eT    :
                tmp = adc_usr.dat_unit_factory * 1;
                break;
            case eF    :
                tmp = adc_usr.dat_unit_factory - 273.15;
                break;
            default    :
                result = 0;
                break;
        }
    }
    else
        result = 0;
    if (factory_unit_convert(tmp) == 1)//pa or T --->user unit
        result = 1;
    else
        result = 0;
    return result;
}

void dat_cal_proc(float dat)
{
    uint32_t tmp;
    if (factory_unit_convert(dat) == 1)//factory unit to pa or T
    {

        tmp = *((uint32_t *)& adc_usr.data_unit_app);
        GetReg()->pb[eREG_HF16].val_f = tmp;
        GetReg()->pb[eREG_LF16].val_f = tmp;
		GetReg()->pb[eREG_LF16].val_f = GetReg()->pb[eREG_LF16].val_f -
			GetReg()->pb[eREG_RANGZ_HF16].val_f;
		GetReg()->pb[eREG_LF16].val_f = GetReg()->pb[eREG_LF16].val_f *
			                   GetRegPrivate()->coe;
        tmp = adc_usr.data_unit_app * 10;
        if (tmp & 0x8000)
        {
            tmp = ~tmp + 1;
            GetReg()->pb[eREG_X10].val_i = tmp;
            GetReg()->pb[eREG_X10].val_i = -GetReg()->pb[eREG_X10].val_i;

        }
        else
            GetReg()->pb[eREG_X10].val_i = tmp;
        GetReg()->pb[eREG_X100].val_i = GetReg()->pb[eREG_X10].val_i * 10;
    }
}
void adc_proc(void)
{
    float tmp1, tmp2, tmp3;
    if (g_cs1237_device_st.reconfig == 1)
        adc_config();
    else
    {
        calculate_adc_num(&g_cs1237_device_st);
        adc_usr.adc_ori = g_cs1237_device_st.adc_calculate_deal_data;

        tmp1 = adc_usr.adc_ori * adc_usr.adc_ori;
        tmp2 = tmp1;
        tmp1 = tmp1 * adc_usr.adc_ori;
        tmp3 = adc_usr.adc_ori;
        tmp3  = GetRegPrivate()->coe1 * tmp1;
        tmp3 = tmp3 + tmp2 * GetRegPrivate()->coe2;
        tmp3 = tmp3 + tmp1 * GetRegPrivate()->coe3;
        adc_usr.dat_unit_factory = tmp3 + GetRegPrivate()->coe4;
        dat_cal_proc(adc_usr.dat_unit_factory);

    }

}
void pwm_ctrl(float ratio)
{
    unsigned int pulse;
    float tmp;
    switch (GetRegPrivate()->sei)
    {
        case 0:
            tmp = 4.0 / 5;
            break;
        case 1:
            tmp = 0.4 / 5;
            break;
        default:
            tmp = 0;
            break;

    }

    pulse = tmp * PWM_COUNTER;
    if (pulse <= PWM_COUNTER)
        GTIM_SetCompare3(CW_GTIM1, pulse - 1); // 1*4   1khz 1000  250-1
}

