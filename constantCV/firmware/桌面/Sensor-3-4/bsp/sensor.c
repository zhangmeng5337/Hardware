...
//                getPgaToADC(GetRegPrivate()->pga), DEV_CH_A);
    ...
    cs1237_set_config(2, GetReg()->pb[eREG_ADC_RATE].val_u32ToFloat + 1);//40Hz
    cs1237_set_config(0, 0);//Channel A
    // cs1237_set_config(0, 3);//?迆2??足?車2a那?   16771616 -5605
    cs1237_set_config(1, getPgaToADC(GetRegPrivate()->pga));//PGA=128
 cs1237_set_config(3, 0);//PGA=128
    ...
    else
    {
        if (system_loop() != 0)
        {
		 adc_usr.adc_ori = g_cs1237_device_st.adc_data;
          tmp1 =  g_cs1237_device_st.adc_data;
            g_cs1237_device_st.adc_calculate_deal_data = tmp1;
...
void cal_press(void)
{
    ...
    float tmp1, tmp2, tmp3;
    tmp2 = g_cs1237_device_st.adc_calculate_deal_data;

    adc_usr.adc_ori = tmp2 / 4;
