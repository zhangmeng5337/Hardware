...
//   GPIO_SetBits(PORT_SX127X_TX_CTRL, PIN_SX127X_TX_CTRL);
  //  
  //  GPIO_Init( GPIOA, GPIO_Pin_0|GPIO_Pin_1, GPIO_Mode_In_FL_No_IT );
  //  GPIO_Init( GPIOA, GPIO_Pin_2|GPIO_Pin_3, GPIO_Mode_In_FL_No_IT );
  //  
  ////  //GPIOB
  //  //GPIO_DeInit(GPIOB);
  //  GPIO_Init( GPIOB, GPIO_Pin_All, GPIO_Mode_Out_PP_Low_Slow );
  ...
  // GPIO_Init(PORT_SPI, PIN_MOSI, GPIO_Mode_Out_PP_High_Slow);  // MOSI (PB6)
// GPIO_Init(PORT_SPI,PIN_SX127X_CSN, GPIO_Mode_Out_PP_Low_Slow);    
 //  GPIO_SetBits( PORT_SX127X_CSN, PIN_SX127X_CSN ); 
   GPIO_ResetBits(PORT_SX127X_RST, PIN_SX127X_RST);   
  ////  //GPIOC
  //  GPIO_Init( GPIOC, GPIO_Pin_All, GPIO_Mode_Out_PP_Low_Slow );
  //  GPIO_Init(PORT_SX127X_DIO3, PIN_SX127X_DIO3, GPIO_Mode_Out_PP_High_Slow); 
  //  GPIO_Init(PORT_SX127X_DIO4, PIN_SX127X_DIO4, GPIO_Mode_Out_PP_Low_Slow);
  //  GPIO_Init(PORT_SX127X_DIO5, PIN_SX127X_DIO5, GPIO_Mode_Out_PP_Low_Slow);
  ...
  //  GPIO_Init(PORT_SX127X_DIO2, PIN_SX127X_DIO2, GPIO_Mode_Out_PP_Low_Slow);
...
{	
  ...
  SystemClock_Init();     // 系统时钟初始化
  disableInterrupts(); 
  //SYSCFG_RIResistorConfig(RI_Resistor_400KPU, ENABLE);
  //SYSCFG_RIResistorConfig(RI_Resistor_400KPD, ENABLE);
  GPIO_Initial();         // 初始化GPIO
  LED_Init();
  SPI_Initial();
  CurrentMode =PowerSavingMode ;//NormalMode  PowerSavingMode
