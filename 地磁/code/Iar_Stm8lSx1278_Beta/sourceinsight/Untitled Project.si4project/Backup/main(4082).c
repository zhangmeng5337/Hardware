...
void main(void)
{
    ...
    while(1)
    {
        ...
        if((GetModuleMode()->LoraM0Flag == 0) && (GetModuleMode()->LoraM1Flag == 0))
            ...
            GetModuleMode().CurrentMode = GetModuleMode().WakeMode;
        ...
        switch (PowerSavingMode) {
            case NormalMode:
                if(ConfigureDone) {
                    ...
                    AUX_CONFIGURING();
                    Uart1_Init(GetModuleParams().SerialRate, CheckBit);                           //设置串口波特率
            ...
            case WakeMode:
                if(ConfigureDone) {
                    ...
                    if( UsartReceiveFlag == 1 )
                    {
                        SX1276_SetPreambleSize((pream_long[GetModuleParams().AirRate]*(GetModuleParams().GetModuleParams().WakeupTime/250)));   //前导码为972 BW 500kHZ SF为7 则空中传输时间250ms
                        SX1276_SetRxMode();
                        SX1276_SendPacket(UsartReceiveData, (usart_i-1));
                        USART_SendData8(USART1, (usart_i-1));
                        while(USART_GetFlagStatus(USART1, USART_FLAG_TXE)==0);
