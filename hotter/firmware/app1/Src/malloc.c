...
//内存池(4字节对齐)
__align(4) unsigned char membase[MEM_MAX_SIZE];                                                    //内部SRAM内存池
//内存管理表
unsigned int memmapbase[MEM_ALLOC_TABLE_SIZE];                                                    //内部SRAM内存池MAP
//内存管理参数
...
void mymemset(void *s,unsigned char c,uint32_t count)
{
    unsigned char *xs = s;
    while(count--)*xs++=c;
}
...
//offset:内存地址偏移
//返回值:0,释放成功;1,释放失败;
unsigned char mem_free(uint32_t offset)
{
    int i;
