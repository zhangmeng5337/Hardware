...
void anlysis_mqtt_recv()
{
    ...
    if (Find_string((char *)Lpuart1type.Lpuart1RecBuff, "设备ID:  ", "\r\n",
                    dev_id) == 1)
    {
        if ((strcmp(dev_id, get_params()->user_id)) == 0)
            valid_flag = 1;
    ...
    if (valid_flag == 1)
    {
                        ...
                        dev_id) == 1)
        {
            ...
            sprintf(&get_params()->update_firm, "%s", dev_id);//字符串
        ...
        {
            get_params()->update_setting = 1;
            sprintf(&get_params()->reboot, "%s", dev_id); //字符串
        ...
        {
            get_params()->update_setting = 1;
            sprintf(&get_params()->machine, "%s", dev_id); //字符串
...
void upload()
{
    ...
    sprintf(mqtt_payload_u.devid, "%s", get_params()->user_id);//devid
