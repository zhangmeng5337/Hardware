				...
				if((HAL_GetTick()-gettime)>=RECYCLE_TIME)
				{
					...
					{				
						...
						case STAND_MODE:       
										...
										tmp[2]=SEG_A;
										tmp[3]=SEG_D;
						                display_dat_deal(settings_params.tar_env_temper[0],tmp,4);break;
						case COLD:				tmp[0]=SET_AT;display_dat_deal(settings_params.tar_env_temper[0],tmp,3);break;
						case HOT:				tmp[0]=SET_AT;display_dat_deal(settings_params.tar_env_temper[1],tmp,3);break;
						case COLD_HUM_COLD:
						case COLD_HUM_HUM: 	
							if(toggle_flag ==0)
							{
