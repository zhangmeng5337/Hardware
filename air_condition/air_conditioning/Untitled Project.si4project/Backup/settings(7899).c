...
void write_dipay_ram(unsigned char umode)
{
		...
		if((HAL_GetTick()-gettime)>=RECYCLE_TIME)
		{
			...
			{			   
				...
				case STAND_MODE: 	  
					...
					tmp[2]=SEG_A;
					tmp[3]=SEG_D;
					display_dat_deal(settings_params.tar_env_temper,tmp,4);break;
				case COLD:			   tmp[0]=SET_AT;display_dat_deal(settings_params.tar_env_temper,tmp,3);break;
				case HOT:			   tmp[0]=SET_AT;display_dat_deal(settings_params.tar_env_temper,tmp,3);break;
				case COLD_HUM_COLD:
				case COLD_HUM_HUM:  
					if(toggle_flag ==0)
					{
   ...
   unsigned char tar_env_temper[4];//4:target cold and humid humid 5:fast humid
   unsigned char equip_env_temper;
