...
void Get_Adc_Average(unsigned char times)
{
	...
	{
	    if(t==3)
			adc_io.adc_result[t]=(float)(get_temperature(adcBuf[t],HUMID,adc_io.adc_result[0]));
		else
			adc_io.adc_result[t]=(float)(get_temperature(adcBuf[t],TEMPERATURE,adc_io.adc_result[0]));			
	}
...
float get_temperature(uint16_t data,unsigned char SensorTypes,float env_temper)
{
	...
	if(SensorTypes == TEMPERATURE)
	{
		if( (data<=NTCTAB[0]) && (data>NTCTAB[NTC_SIZE-1]) )
			{
				num=look_up_table((u16 *)NTCTAB,index,data);
	...
	if(SensorTypes == HUMID)
	{
		...
		if( (data<=HLTAB[ index][0]) && (data>HLTAB[ index][HL_SIZE_COLUMB-1]) )
			{
				num=look_up_table((u16 *)HLTAB,HL_SIZE_COLUMB,data);
				t1=num_to_temperature(num);
