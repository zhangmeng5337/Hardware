...
void led_ctrl(unsigned char num,unsigned char on_off)
{
	if(on_off == ON)
	{
			switch(num)
			{
				...
				case HIGH_LEVEL:  HAL_GPIO_WritePin(led_ctrl1_GPIO_Port, led_ctrl1_Pin,GPIO_PIN_RESET);break;
				case LOW_LEVEL:   HAL_GPIO_WritePin(led_ctrl2_GPIO_Port, led_ctrl2_Pin,GPIO_PIN_RESET);break;		
				case LED_ALL:   
