...
adc_io.adc_result[1]=26.5;
	for(t=0;t<5;t++)
	{
	    if(t==0)
			adc_io.adc_result[t]=(float)(get_temperature(adcBuf[t],HUMID,adc_io.adc_result[1]));
		else
...
float get_temperature(uint16_t data,unsigned char SensorTypes,float env_temper)
{
	...
	if(SensorTypes == HUMID)
	{
		...
		if( (data<=HLTAB[ index][0]) && (data>HLTAB[ index][HL_SIZE_COLUMB-1]) )
			{
				num=look_up_table((u16 *)(HLTAB[index][16]),HL_SIZE_COLUMB,data);
