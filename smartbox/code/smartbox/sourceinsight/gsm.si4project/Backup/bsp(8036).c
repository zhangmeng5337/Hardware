...
void Init_Timer1(void)
{
   ...
   // TIM1_CCxCmd(TIM1_Channel_2, ENABLE);                   //使能输入捕获通道2
    TIM1_Cmd(ENABLE);*/                                      //使能定时器
	 ...
	 TIM1_DeInit(); 										//tim1 定时器寄存器全部复位  
	 TIM1_TimeBaseInit(0x0f, TIM1_CounterMode_Up, 50000-1, 0);//	10s  最小捕获50.2hz 	 设置时间分频  向上计数	 捕捉50Hz及 以上的周期和占空比
	 TIM1_PWMIConfig(TIM1_Channel_1, TIM1_ICPolarity_Falling, TIM1_ICSelection_DirectTI, TIM1_ICPSC_DIV1, 0x00); //选择TI1输入上升沿触发 选择TI2输入下降沿触发 输入捕捉预定标器  无滤波
	 TIM1_SelectInputTrigger(TIM1_TRGSelection_TI1FP1);   //滤波后输入TI1
	 TIM1_SelectSlaveMode(TIM1_SlaveMode_Reset);//复位输入
	 TIM1_ClearFlag(TIM1_FLAG_CC1); 						//指定要清除的标志
	 TIM1_ClearFlag(TIM1_FLAG_CC2);						  //指定要清除的标志
