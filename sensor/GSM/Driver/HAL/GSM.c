...
unsigned char SIMCOM_GetStatus(unsigned char *p,unsigned char *ack,uint32_t waittime)
{
  ...
  if(waittime)		//需要等待应答
  {
    ...
    if(waittime==0){uart_str.receive_flag=0;
      res=0;
      //memset(uart_str.UsartReceiveData,0,256);
    }  
  return res;
...
void SIMCOM_Register_Network()
{
 ...
 // SIMCOM_ReConnect();
  ...
  case SIMCOM_ATE0:
    {
      if(SIMCOM_GetStatus((unsigned char*)Check_SIM,(unsigned char*)Respond_CPIN,10000)==1)
  ...
  case SIMCOM_SIM_OK:
  {
	if(SIMCOM_GetStatus((unsigned char*)Normal_ModeT,(unsigned char*)Respond_Normal_ModeT,100000)==1)
	{//  NET_STAUS=SIMCOM_SIM_OK;
        memset(uart_str.UsartReceiveData,0,256);
        NET_STAUS=SIMCOM_SIM_OK2;}
  }
  break;
  case SIMCOM_SIM_OK2:
  {
	if(SIMCOM_GetStatus((unsigned char*)Network_Intensity,(unsigned char*)Respond_Network_Intensity,60000)==1)
	{//  NET_STAUS=SIMCOM_SIM_OK;
        memset(uart_str.UsartReceiveData,0,256);
        NET_STAUS=SIMCOM_Network_Intensity_READY;
        }
