  ...
  if(waittime)		//需要等待应答
  {
    while(--waittime)	//等待倒计时
    {
      if(uart_str.receive_flag==1)//接收到期待的应答结果
      {
        ...
        else  if(AT_cmd_ack(&uart_str.UsartReceiveData[0],(unsigned char*)Respond_No_Carrier)
...
void SIMCOM_Register_Network()
{
  ...
  case SIMCOM_SIM_OK:
    {
      if(SIMCOM_GetStatus((unsigned char*)Normal_ModeT,(unsigned char*)Respond_Network_Normal_ModeT,100000)==1)
      { //NET_STAUS=SIMCOM_SIM_OK;
  ...
  case SIMCOM_NET_TRANSPARENT:
    {
      //server_ip_tmp=Establish_TCP_Connection;
      if(SIMCOM_GetStatus((unsigned char*)server_ip,(unsigned char*)Respond_TCP_Connect,90000)==1)
      {
  ...
  case SIMCOM_NET_OK:
    {
      ...
      {  
        if(SIMCOM_GetStatus((unsigned char*)LAST_CMD,(unsigned char*)Respond_TCP_Connect,80000)==1)
        {
          NET_STAUS=SIMCOM_NET_OK;
          memset(uart_str.UsartReceiveData,0,buffer_size);
        }         
      }
      else if(SIMCOM_Get_TCP_Staus(40)==2)
      {  
