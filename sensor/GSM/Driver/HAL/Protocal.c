...
void module_prams_init()
{
  ...
  if(tmp==0x5aa55a)
    Data_usr.Warn_Thres = Data_usr.calibration+500;
  else 
    Data_usr.Warn_Thres = 0x25;
...
void OilCalibration()
{
  ...
  adc_sum = adc_sum /100;
  Data_usr.calibration = adc_sum;
  
  adc_tmp = 0x5aa55a;
  flash_operation(FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS,&adc_tmp,1);
  flash_operation(FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS+4,&adc_sum,1);
...
void module_process()
{
  ...
  if(Get_Network_status()==SIMCOM_NET_OK)
  {
    ...
    Data_usr.vbat[0] = (unsigned char)(adc_tmp/1000)*2;
    Data_usr.vbat[1] = (unsigned char)(adc_tmp/100%10)*2;
