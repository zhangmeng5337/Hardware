...
void SIMCOM_Register_Network()
{
  ...
  case SIMCOM_READY_YES://SIMCOM_READY_YES:
    {
      if( SIMCOM_GetStatus((unsigned char*)Echo_Dis,(unsigned char*)Respond_OK,20000)==1)
  ...
  case SIMCOM_ATE0:
    {
      if(SIMCOM_GetStatus((unsigned char*)Check_SIM,(unsigned char*)Respond_CPIN,30000)==1)
  ...
  case SIMCOM_SIM_OK:
  {
	if(SIMCOM_GetStatus((unsigned char*)Network_Intensity,(unsigned char*)Respond_Network_Intensity,40000)==1)
	//  NET_STAUS=SIMCOM_SIM_OK;
        NET_STAUS=SIMCOM_Network_Intensity_READY;
  }
  break;
    ...
    case SIMCOM_Network_Intensity_READY:
    {
      //if(SIMCOM_GetStatus((unsigned char*)GPRS_Attached_State,(unsigned char*)Respond_Attached_Ok,30000)==1)
        NET_STAUS=SIMCOM_GPRS_READY;
    }
    break;
