...

{
  ...
  case 4:
	...
	if(digit_flag == 4)
    {
        if((tmp&d)==0)
        {
		LCD_RAMX[LCD_RAMRegister_11]&=~0x40;
		LCD_RAMX[LCD_RAMRegister_11]|=0x08;
		}
		else
		{
			LCD_RAMX[LCD_RAMRegister_11]|=0x48;	
    		LCD_WriteRAM(LCD_RAMRegister_11,LCD_RAMX[LCD_RAMRegister_11]);//d
    	}
  ...
  void lcd_process()
  {
    LCD_DisplayNum(11234);delay_ms(2000); 
    LCD_DisplayNum(1456.7);delay_ms(2000);
    LCD_DisplayNum(178.90);delay_ms(2000);
    LCD_DisplayNum(15.886);delay_ms(2000);
    LCD_DisplayNum(1.7362);delay_ms(2000);
  }