...
unsigned int RepeatTime=10;//10:9s，修改休眠时间
float warn_setting = 3.5;
...
#define VBAT_THRES    5500000          
  ...
  #endif
 {
  if(RtcWakeUp==1||Data_usr.status == 1)
  {
    ...
    // flow_get();
    if((Data_usr.vbatf >=VBAT_THRES&&Data_usr.status == 1)||
       (Data_usr.vbatf<VBAT_THRES&&RtcWakeUp==1))
    data_tansmmit();
    RtcWakeUp = 0;
    //Set_Network_status();
	//set_NetStatus(SIMCOM_NET_NOT);
    #if module == sensor
    if(Data_usr.vbatf >=VBAT_THRES)
    {
