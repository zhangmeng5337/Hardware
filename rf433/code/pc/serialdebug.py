...
#增加标签
ttk.Label(frameUpDown,text="上下关系").grid(column=50,row=0,padx = OpadxSpace,pady =0)
...
cmbChosen.grid(column = 1, row = 0,padx = OpadxSpace,pady =OpadySpace)
...
if len(plist) <= 0:
  ...
  for i in range(0,len(plist)) :
   ...
   port_serial = port_list_0[i]
   #ser = serial.Serial(port_serial,9600,timeout = 60)
   print("serial count>",i)
   print("chek which port was really used>",plist[i])
   
   cmbChosenOUT4['value']=(plist[i],'不拟制')
   cmbChosenOUT4.current(0)  
