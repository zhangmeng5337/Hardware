...
void RS485_Init(void)
{
  ...
  huart1.Init.OverSampling = UART_OVERSAMPLING_16;
        ...
        {
                case 0:huart1.Init.Parity=UART_PARITY_NONE;break;//无校验
                case 1:huart1.Init.Parity=UART_PARITY_ODD;break;//奇校验
                case 2:huart1.Init.Parity=UART_PARITY_EVEN;break;//偶校验
        }
...
void RS485_Service(void)
{
        ...
        if(modbus_usr.RS485_FrameFlag==1)
        {
                ...
                {
                        if((modbus_usr.RS485_RX_BUFF[1]==01)||(modbus_usr.RS485_RX_BUFF[1]==02)||(modbus_usr.RS485_RX_BUFF[1]==03)||(modbus_usr.RS485_RX_BUFF[1]==05)||(modbus_usr.RS485_RX_BUFF[1]==06)||(modbus_usr.RS485_RX_BUFF[1]==15)||(modbus_usr.RS485_RX_BUFF[1]==16))//功能码正确
                  {
...
void Modbus_03_Solve(void)
{
        ...
        if((startRegAddr+RegNum)<1000)//寄存器地址+数量在范围内
        {
                ...
                for(i=0;i<RegNum;i++)
                {
                        modbus_usr.RS485_TX_BUFF[3+i*2]=0;//(*Modbus_HoldReg[startRegAddr+i]>>8)&0xFF;//           /////////先发送高字节--在发送低字节
                        modbus_usr.RS485_TX_BUFF[4+i*2]=0;//(*Modbus_HoldReg[startRegAddr+i])&0xFF; //
                }
