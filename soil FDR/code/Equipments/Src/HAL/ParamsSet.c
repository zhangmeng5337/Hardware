...
void loraTransmit(unsigned char lora_num,unsigned char mode,unsigned char *q,unsigned char len)
{
	if(lora_num == LORA_MASTER)
	switch(mode)
	{
		case 0: loraGpioSet(LORA_MASTER,mode);
		        loratxdata(LORA_MASTER,q,len);
		       break;
		case 1: break;		
		case 2: break;
		case 3: break;		
...
void loraModeConfig(unsigned char lora_num,unsigned char mode,unsigned char startaddr,unsigned char len,unsigned char wr)
{
  ...
  len_sum = len_sum + len; 	
	loraTransmit(lora_num,mode,p,len_sum);
}
