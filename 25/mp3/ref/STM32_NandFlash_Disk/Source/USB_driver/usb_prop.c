...
void MASS_Reset()
{
  ...
  SetBTABLE(BTABLE_ADDRESS);  //设置分组缓冲区描述表起始地址

  /* Initialize Endpoint 0 */
  SetEPType(ENDP0, EP_CONTROL);	  	//设置为控制端点
  SetEPTxStatus(ENDP0, EP_TX_NAK);	 //设置发送端点状态
  SetEPRxAddr(ENDP0, ENDP0_RXADDR);	  //设置接收缓冲区地址
  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);
  SetEPTxAddr(ENDP0, ENDP0_TXADDR);	   //设置发送缓冲区地址
  Clear_Status_Out(ENDP0);
  SetEPRxValid(ENDP0);

  /* Initialize Endpoint 1 */
  SetEPType(ENDP1, EP_BULK);	   //设置为批量端点
  SetEPTxAddr(ENDP1, ENDP1_TXADDR);	  //发送缓冲区地址
  SetEPTxStatus(ENDP1, EP_TX_NAK);	  //端点以NAK分组响应所有的发送请求
  SetEPRxStatus(ENDP1, EP_RX_DIS);	 //忽略接收请求

  /* Initialize Endpoint 2 */
  SetEPType(ENDP2, EP_BULK);
  SetEPRxAddr(ENDP2, ENDP2_RXADDR);
  SetEPRxCount(ENDP2, Device_Property.MaxPacketSize);
  SetEPRxStatus(ENDP2, EP_RX_VALID);  //端点可用于接收
  SetEPTxStatus(ENDP2, EP_TX_DIS);	//忽略发送请求
