...
//GPIO_Init( GPIOC, GPIO_Pin_7, GPIO_Mode_In_PU_No_IT );
  GPIO_Init( PORT_MOTORP, PIN_MOTORP, GPIO_Mode_Out_PP_Low_Fast );
  GPIO_Init( PORT_MOTORN, PIN_MOTORN, GPIO_Mode_Out_PP_Low_Fast ); 
  GPIO_Init( PORT_MOTOR_STATUS_OPEN, PIN_MOTOR_STATUS_OPEN, GPIO_Mode_In_PU_No_IT );
  GPIO_Init( PORT_MOTOR_STATUS_CLOSE, PIN_MOTOR_STATUS_CLOSE, GPIO_Mode_In_PU_No_IT ); 
...
{   
	...
	result = 0;
	if(GPIO_ReadInputDataBit(PORT_MOTOR_STATUS_OPEN, PIN_MOTOR_STATUS_OPEN)==0)
		result = result |0x01;
	else
		result = result &0xfe;
	if(GPIO_ReadInputDataBit(PORT_MOTOR_STATUS_CLOSE, PIN_MOTOR_STATUS_CLOSE)==0)
		result = result |0x02;
	else
		result = result &0xfd;
...
unsigned char motor_ctrl(unsigned char dir)
{
   ...
   if(dir == OPEN)
   	{
   		GPIO_SetBits( PORT_MOTORP, PIN_MOTORP );
   		GPIO_ResetBits( PORT_MOTORN, PIN_MOTORN );
		while(get_motor_status() != OPEN)
		{
			timeout ++;
			if(timeout>=100000)
			{
				timeout = 0;
				GPIO_SetBits( PORT_MOTORP, PIN_MOTORP );
				GPIO_SetBits( PORT_MOTORN, PIN_MOTORN );
				result = 2;
				break;
			}
		}
...
uint32_t adcGet(ADC_Channel_TypeDef num,unsigned int samplecount)
{
  ...
  ADCdata = (uint32_t)(tmp2*1.0 /samplecount);
  ADC_DeInit(ADC1);
