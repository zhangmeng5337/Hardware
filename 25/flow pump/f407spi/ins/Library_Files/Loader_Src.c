...
//uint32_t SystemCoreClock = 16000000;  // 初始值可以设为16MHz
...
{ 
  FM25W32_ReadData(Address, buffer, Size);
  return 1;
} 
...
{      
  ...
  //  sFLASH_EraseSector(EraseStartAddress);
	FM25W32_SectorErase(EraseStartAddress);
...
uint32_t CheckSum(uint32_t StartAddress, uint32_t Size, uint32_t InitVal)
{
  ...
  {
    FM25W32_ReadData(StartAddress,&value, 1);
    Val = value;
	FM25W32_ReadData(StartAddress+1,&value, 1);
   // sFLASH_ReadBuffer(&value, StartAddress + 1,1);
    Val+= value<<8;
	FM25W32_ReadData(StartAddress+2,&value, 1);
   // sFLASH_ReadBuffer(&value, StartAddress + 2,1);
    Val+= value<<16;
	FM25W32_ReadData(StartAddress+3,&value, 1);
...
uint64_t Verify (uint32_t MemoryAddr, uint32_t RAMBufferAddr, uint32_t Size, uint32_t missalignement)
{
  ...
  while (Size>VerifiedData)
  {
    ...
    FM25W32_ReadData( MemoryAddr+VerifiedData,&TmpBuffer, 1);     
