...
void adc_init(void)
{
  ...
  //sensor_adc_value.v2_ratio =V4_V;//pilot 3 
	sensor_adc_value.v2_ratio =V2_V;//pilot 
...
void temperature_proc()
{
    ...
    sensor_adc_value.average_T = tmp - get_config()->tC;
    tmp = average_filter(sensor_adc_value.T_value[2], 1);
    sensor_adc_value.average_P1_T = tmp - get_config()->tC;
        ...
        if (sensor_adc_value.update == 1)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                if (SENSOR_TYPE == PT)   //v1r1/(v2(1+r1/r2)-v1)
                {
                    ...
                    x2 = x2 + VOL_FACTOR * sensor_adc_value.v2_ratio; //v1  0.064 bridge voltage
                    x2 = x2 / VOL_FACTOR;
                    sensor_adc_value.v_value[i] = x2;
                    x2 = (PT_VREF - x2);
                    x2 = x2 / R2_V;
                    x1 = sensor_adc_value.v_value[i] / x2;
                }
                else
                    adc_conv_tmp = adc_conv_tmp / VOL_FACTOR ;
                uint32_t tmp_r;
                tmp_r = x1 * 1000000;
                sensor_adc_value.R_value[i] = tmp_r/1000.0;
