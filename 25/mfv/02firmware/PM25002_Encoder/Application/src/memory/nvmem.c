...
uint32_t rtb_waddress = 0;  // current address to write on
uint32_t rtb_eraseaddress = 0; // next address to write on
uint32_t rtb_src_addr;
uint8_t rtb_wblk = 0;
uint8_t rtb_state_machine = 0;
rtb_struct rtb;

uint8_t az_save_now = 0;
uint32_t az_waddress = 0;  // current address to write on
uint32_t az_eraseaddress = 0; // next address to write on
uint32_t az_src_addr;
uint8_t az_wblk = 0;
uint8_t az_state_machine = 0;
az_struct autoz;

res_states res_sm = RES_INIT;
uint32_t resAddress = 0;
uint32_t src_res_addr;
uint32_t res_wr = 0;

uint8_t eccDDoubleBitErrOccured = 0;
uint8_t eccDoubleBitErr = 0;
...
/**
    @brief start counting down before we write to flash. Timeout is in milliseconds
*/
void rebuildRedundantPage(uint16_t timeout) {
#ifndef BOOTLOADER  
    if (!flash_busy) 
    { // make sure that there are no write in progress
...
void load_default_values(nvmem_struct *nv) {
  ...
  strncpy((char *)nv->app.manufacturing.senserialnumber, "SN__", sizeof(nv->app.manufacturing.senserialnumber));
  strncpy((char *)nv->app.manufacturing.devserialnumber, "DV__", sizeof(nv->app.manufacturing.devserialnumber));
...
void load_data_from_flash(void) {
...
#ifdef BOOTLOADER
  nvmem.app.network.defaultbaudrate = 2;  // 19200
  nvmem.app.network.length = 1;
  nvmem.app.network.parity = 0;
  nvmem.app.network.stopbits = 1;
  nvmem.app.network.address = 1;
  nvmem_state_machine = NVMEM_IDLE;
#else  
  eccDDoubleBitErrOccured = 0; // reset ECCD/ECCD2 flag
  // load main non-volatile memory page
  memcpy(&nvmem, (uint8_t*)FLASH_USER_START_ADDR, sizeof(nvmem));  
  if (eccDDoubleBitErrOccured > 0) {
    eccDoubleBitErr |= STATUS_MEM_MAIN_BAD;
  }  
  ...
  if (eccDDoubleBitErrOccured > 0) {
    eccDoubleBitErr |= STATUS_MEM_REDUNDANT_BAD;
  }
  if (nvmem.magic_number == FLASH_MAGICNUMBER) {
  ...
  if (page_stat == 0x00) {  // problem with both pages. lets start fresh
    ...
    if ((page_stat & 0x01) && (lcl_crc16_main == flash_crc16_main)) {
      ...
      if (lcl_crc16_red != flash_crc16_red) {
        rebuildRedundantPage(FLASH_TIMEOUT);  // initiate a redundant page resotration
      } else {
        nvmem_state_machine = NVMEM_IDLE;
      }
      ...
      // load main non-volatile memory page from restore point
      memcpy(&nvmem, (uint8_t*)(FLASH_FACTORY_START_ADDR), sizeof(nvmem));
      write_to_flash(FLASH_TIMEOUT);
    }
    #ifndef BOOTLOADER
...
/**
  @brief FACTORY Resotre Point
*/
res_states restore_stat_machine(void) {
  uint16_t lcl_crc16;
  static FLASH_EraseInitTypeDef EraseInitStruct; 
  __IO uint64_t *src;
  nvmem_struct *resptr = (nvmem_struct*)FLASH_FACTORY_START_ADDR;
  
  switch (res_sm) {
    case RES_INIT:
      // check if we have a valid restore point. if all FF's, create one
      if (resptr->magic_number == 0xFFFFFFFF) {
        // lets create a restore point
        res_wr = FLASH_FACTORY_MAGICNUMBER;
      } 
      res_sm = RES_WAITING;
      break;
    case RES_WAITING:
      if (flash_busy) break;  // wait for flash not to be busy before we do anything
      // wait for a trigger to save a restore point
      if (res_wr == FLASH_FACTORY_MAGICNUMBER) {
        /* Unlock the Flash to enable the flash control register access *************/
        HAL_FLASH_Unlock();      
         /* Clear OPTVERR bit set on initial samples */
        __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_OPTVERR);
        __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_PGSERR);
        /* Get the 1st page to erase */
        FirstPage = GetPage(FLASH_FACTORY_START_ADDR);
        /* Get the number of pages to erase from 1st page */
        NbOfPages = GetPage(FLASH_FACTORY_END_ADDR) - FirstPage + 1;       
        /* Get the bank */
        BankNumber = GetBank(FLASH_FACTORY_START_ADDR);
        /* Fill EraseInit structure*/
        EraseInitStruct.TypeErase   = FLASH_TYPEERASE_PAGES;
        EraseInitStruct.Banks       = BankNumber;
        EraseInitStruct.Page        = FirstPage;
        ...
        if (HAL_FLASHEx_Erase(&EraseInitStruct, &PageError) == HAL_OK) {
          nvmem.crc16 = crc16_ccitt(FLASH_CRC_SEED, (uint8_t*)&nvmem, sizeof(nvmem)-2);							
          src_addr = (uint32_t)&nvmem.magic_number;
          Address = FLASH_FACTORY_START_ADDR;
          res_sm = RES_WRITE;
        }
      } else if (res_wr == FLASH_RESTORE_MAGICNUMBER) {
        res_sm = RES_RESTORE;
      }
      break;
    case RES_WRITE:  
      if (Address < (FLASH_FACTORY_START_ADDR + sizeof(nvmem))) {
        flash_it = false;
        src = (__IO uint64_t*)src_addr;
        HAL_FLASH_Program_IT(FLASH_TYPEPROGRAM_DOUBLEWORD, Address, *src);
        res_sm = RES_WAIT_WRITE;          
      } else {
        res_sm = RES_VALIDATE;
      }      
      break;
    ...
    case RES_RESTORE:
      memcpy(&nvmem, (uint8_t*)(FLASH_FACTORY_START_ADDR), sizeof(nvmem));
      res_sm = RES_RESTORE_FINALIZE;
      break;
    case RES_RESTORE_WRITE:  
    case RES_RESTORE_WAIT_WRITE:
    case RES_RESTORE_VALIDATE:
    case RES_RESTORE_FINALIZE:
      //HAL_FLASH_Lock();   
      res_sm = RES_WAITING;
      res_wr = 0;
      flash_busy = false; 
      __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_OPTVERR);
      __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_PGSERR);
      write_to_flash(FLASH_TIMEOUT);  // initiate the restore write
      break;
  }
