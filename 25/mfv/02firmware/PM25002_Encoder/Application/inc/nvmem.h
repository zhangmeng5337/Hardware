...
#define ADDR_FLASH_PAGE_250   ((uint32_t)0x0807d000) /* Base @ of Page 250, 2 Kbytes -- expansion slot */
...
#define ADDR_FLASH_PAGE_255   ((uint32_t)0x0807f800) /* Base @ of Page 255, 2 Kbytes -- expansion slot */

#define NVIC_VectTab_RAM             ((uint32_t)0x20000000)
#define NVIC_VectTab_FLASH           ((uint32_t)0x08000000)
#define IS_NVIC_VECTTAB(VECTTAB) (((VECTTAB) == NVIC_VectTab_RAM) || \
                                  ((VECTTAB) == NVIC_VectTab_FLASH))
#define VECTOR_TABLE_OFFSET         (uint32_t)0x10000
#define FLASH_APP_MAGICNUMBER       ((uint32_t)0x04262003)   /* FLASH APP magic number */
#define FLASH_APP_HEADER_ADDR       ADDR_FLASH_PAGE_30   /* Application header location*/
#define FLASH_APP_HEADER_ENDADDR    ADDR_FLASH_PAGE_32 - 1   /* Application header endlocation*/
#define FLASH_APP_START_ADDR        ADDR_FLASH_PAGE_32   /* Start @ of App Flash area */
#define FLASH_APP_END_ADDR          ADDR_FLASH_PAGE_128 - 1   /* End @ of App Flash area */

// Main flash data page ADDR_FLASH_SECTOR_2
#define FLASH_USER_START_ADDR   ADDR_FLASH_PAGE_246
#define FLASH_USER_END_ADDR     (ADDR_FLASH_PAGE_249 + FLASH_PAGE_SIZE - 1)
// Redundant flash data page ADDR_FLASH_SECTOR_3
#define FLASH_USER_REDUNDANT_START_ADDR   ADDR_FLASH_PAGE_251
#define FLASH_USER_REDUNDANT_END_ADDR     (ADDR_FLASH_PAGE_254 + FLASH_PAGE_SIZE - 1)
// RTB blocks
#define FLASH_USER_RTB_START_ADDR         ADDR_FLASH_PAGE_240
#define FLASH_USER_RTB_END_ADDR           ADDR_FLASH_PAGE_242
// Factory flash restore point
#define FLASH_FACTORY_START_ADDR   ADDR_FLASH_PAGE_235
#define FLASH_FACTORY_END_ADDR     (ADDR_FLASH_PAGE_238 + FLASH_PAGE_SIZE - 1)
#define FLASH_FACTORY_MAGICNUMBER   12190609
#define FLASH_RESTORE_MAGICNUMBER   17160708

#define FLASH_USER_AZ_START_ADDR  ADDR_FLASH_PAGE_229
#define FLASH_USER_AZ_END_ADDR    ADDR_FLASH_PAGE_231

#define FLASH_MAGICNUMBER       0x825fab3d
#define FLASH_CRC_SEED          0xdb19
#define FLASH_THREAD_PERIOD     50      // in milliseconds
#define MAX_FLASH_RETRIES       5
#define MAX_CAL_COEFF           5       // 4th order max
#define MAX_LINCURVE_CAL        10
#define FLASH_TIMEOUT           500    // in milliseconds
#define MAX_CARDINAL_PTS        20
#define RTB_SAVE_INTERVAL       1200  // 60 seconds / FLASH_THREAD_PERIOD(ms)
#define AZ_SAVE_INTERVAL       36000  // 1800 seconds / FLASH_THREAD_PERIOD(ms)
...
typedef enum
{
    DNS_DATATYPE_INT  = 0xC3,
    DNS_DATATYPE_REAL  = 0xCA,
} dns_datatype;

typedef enum
{
    DNS_DATAUNITS_COUNTS  = 0x1001,
    DNS_DATAUNITS_SCCM    = 0x1400,
    DNS_DATAUNITS_PERCENT = 0x1007,
} dns_dataunits;
...
typedef enum
{
    ...
    RES_RESTORE,
    RES_RESTORE_WRITE,
    RES_RESTORE_WAIT_WRITE,
    RES_RESTORE_VALIDATE,
    RES_RESTORE_FINALIZE
} res_states;

typedef struct
{
    float x[MAX_CAL_COEFF];         // allow up to MAX_CAL_COEFF-1 order polynomial calibration
} poly_struct;
...
#define MAX_DYNAMIC_GAIN_TABLE  6
typedef struct
{
    float fxn[MAX_DYNAMIC_GAIN_TABLE];
} dyn_controller_gain;

typedef struct
{
    dyn_controller_gain P;
    dyn_controller_gain I;
    dyn_controller_gain SP;
} dyn_controller;

typedef struct
{
    uint8_t mode;
    uint8_t fbsel;  // feedback selector; 0:mdot, 1:vdot
    float pgain;
    float igain;
    float dgain;
    float icrack;
    ...
    float isatl;
    float isath;
    float KppPsi; // Kp per psi
    float KipPsi; // Ki per psi
    float cpPsi;  // crack per psi
    float tPsi;   // tuning pressure
    uint8_t type;
    float tMw;    // tuning MW
    float KipSP;  // Ki per setpoint
    float tSP;    // tuning setpoint
    uint16_t turnondelay;
    uint16_t minramptime;
    dyn_controller dynGain;
    float nPPgain;
    float nPigain;
    float nRange10TorrC1;//below 10torr,error coff
    float nRange30TorrC1;//below 30torr,error coff
    float nRange100TorrC1;//below 100torr,error coff
    float nRange1Sp; //pressure ctrl range 10orr
    float nRange2Sp;//pressure ctrl range 30orr
    float nRange3Sp;//pressure ctrl range 100orr
    float nInternalSpCtrlPoint;//for slope ctrl time
    //float nInternalCtrlRatio;//internal sp changed when interal sp reached
   // float nInternalCtrlSpInterval;//internal sp inc
    float nSlopeCtrlVal;//slope ctral value
    float nSlopeStableTime;//wait for pressure stable for cal slope
    float nSlopeMinPerr1;//wait for pressure stable for cal slope
    float nSlopeMinPerr2;//pressure error for slope cal
    float npidPgianCofDW;
	float npidIgainCofDW;
	//float npidIgainStableCofUP;
	float npidIgainCof;
	float npidPgainCof;	
	float npidIgainCof2;
	float npidPgainCof2;
	float npidPgainCofTh;
	float nslopeCtrlPreTh;
	//float npidPgainStableCofUP;
	//float npidIgainStableCofUP2;	
	float nslopeCrack;
	float nslopeMin1;
	float nslopeMin2;
	float nslopeMin3;	
	...
	float nslopeCtrlL;
	float nslopeCtrlC;
	float nslopeCtrlMinCof;
	float ncofErr;
	float nerrCofTh;
	float nslopeErrCof1;
	float nslopeErrCof2;	
	float nslopeErrCofDiv;
	float nratioCtrlTh;
	float ndownFastSlope;
	float ndownSlowSlope;
	float ndownErrTh;
	float nrateMaxNorm;
	float nrateMinNorm;
	float nerrAllow;
	float nsoftLandTh;
	float nrateSepTh;
	float nrange1ChangeSlopeCof;
	float nrange2ChangeSlopeCof;
	float nrange3ChangeSlopeCof;
	float nRang1PgainCof;
	float nRang2PgainCof;
	float nRang3PgainCof;
	
	float nrampMaxDu;//#define MAX_DU_DT               0.08                     // 阀门最大变化率 (%/s)
	float nrampOscH;//#define OSC_THRESHOLD_HIGH      1.5/10.0                     // 高标准差 (Torr/s)
	float nrampOscL;//#define OSC_THRESHOLD_LOW       0.8/10.0                     // 低标准差 (Torr/s)
	float nrampMeanT;//#define MEAN_TOL                0.2/10                     // 均值接近0的判定 (Torr/s)
	float nrampSteadyTh;//#define STEADY_THRESHOLD        0.3/10                     // 平稳允许标准差 (Torr/s)
	float nrampQTh;//#define Q_THRESHOLD             0.5/10                     // 允许速率误差平滑值 (Torr/s)
	float nrampAdatpInter;//#define ADAPT_INTERVAL          0.2*30                     // 自适应更新周期 (s)
	float nrampInstabliTh;//#define INSTABILITY_THRESHOLD   3.0/10                     // 瞬时幅值保护 (Torr/s)
	float nrampLockTime;//#define LOCK_TIME               2.0                     // 振荡后锁定增益增加时间 (s)
	float nrampPressToler;//#define PRESSURE_TOLERANCE      0.51/1000     // 压力误差 (Torr)
	float nrampRateToler;//#define RATE_TOLERANCE          0.4/10     // 速率波动 (Torr/s)
	float nrampStableCnt;//#define STABLE_CNT_SWITCH       5       // 连续满足周期数
	float nrampKpRate;//#define KP_RATE_LIMIT  0.5/10     // 每秒最大变化量
	float nrampKiRate;//#define KI_RATE_LIMIT  0.1/10     // 相应 Ki 变化率限制
	float nrampKpMin;//#define KP_MIN                  0.5
	float nrampKpMax;//#define KP_MAX                  8.0
	float nrampKiMin;//#define KI_MIN                  0.01
	float nrampKiMax;//#define KI_MAX                  0.8
	float nrampKpOri;//#define KP0                     2.0
	float nrampKiOri;//#define KI0                     0.05
	float nrampMaxDecRate;//#define DECEL_ACCEL  1.5/10   // 允许的最大减速度 (Torr/s²)
	float nrampMagin;//#define DECEL_MARGIN 5.0/1000   // 安全余量 (Torr)
	float nrampMinRate;//#define V_MIN                   0.5/10     // 最小速率截断 (Torr/s)
	float npressLowRangSlopeCof;
...
typedef struct
{
    ...
    uint32_t manufacturererrors;
} latched_error_struct;

typedef struct
{
    uint8_t cfwversion[9];
    uint8_t senserialnumber[32];
    uint8_t electronicsrev[32];
    uint8_t partnumber[40];
    //  uint8_t adcselect;
    //  uint8_t dpalgoselect;
    //  float errRePoly[5];
    //  uint8_t spare00[2];
    uint16_t usVendorId;
    ...
    //  uint8_t dacselect;
    uint8_t devserialnumber[12];
    //  uint8_t devicemode;
    uint8_t invertdrive;
    //  uint8_t totalizerpinctrl;
    uint8_t idledrive;
    //  uint8_t valvetype;
    //  uint8_t spare01[4];
} manufacturing_nvmem_struct;

typedef struct
{
    //  uint8_t spare00;
    uint8_t address;
    uint32_t defaultbaudrate; // 19200
    uint8_t length;           // 8
    uint8_t parity;           // n
    uint8_t stopbits;         // 1
    //  uint8_t spare01[4];
...
}adaptiveKalmanParams_stuct;

typedef struct
{
    lincurve_element lin[8];
} ptCal;
...
typedef struct
{
    ...
    float dx[2][9];
    float fullscale;
    uint32_t adtref;    // reference temperature from calibration
    float madt;         // ADT vs. temp(c) from sensor manufacturer
    float spanpercnt;   // Q3 span per count
    float psipercnt;    // N14 PSI/count
    float adtmin;
    float adtmax;
    float adp0poly[6];  // quadratic ADP(0)
    float calbias;
    uint8_t spare[8];
} pxCal;
...
typedef struct
{
    ...
    //  float pmzero;
    //	float cmpoly[4];
    //  uint8_t autozeroenabled;
    //  float autozerothreshold;
    //  float autozerodelay;
    //  float autozeromdpband;
    //  uint8_t sorientation;
    //  uint8_t fltsel;
    //  uint8_t spare[29];
    //  float nfOffGain0Limit;
    //  float nfOffGain1;
    //  float nfOnGain0Limit;
    //  float nfOnGain0;
    //  float nfOnGain1;
    //  float dt_zero_offset;
    //  float dtck[5];
    kalman_params_struct  fltcdg[2];
    adaptiveKalmanParams_stuct adaKfP[2];
    float sparef[5];
	unsigned char nadjustMode;
	float ncdg1offsetTmp;
	float ncdg1valueNow; 
	float ncdg2offsetTmp;
	float ncdg2valueNow; 
	unsigned char nsensorMode;
} nvflowmeter_struct;
...
typedef struct
{
    uint8_t eflag;     // enable flag
    float tph;         // 0..1
    float tpl;         // 0..1
    float hyst;        // 0..1
    uint16_t SetTime;  // 0..65535
} dns_aw_struct;

typedef struct
{
    dns_datatype dType;      // INT: 0xC3; REAL: 0xCA
    dns_dataunits dUnits;    // Counts: 0x1001; SCCM: 0x1400; PSI: 0x1300; Celsius: 0x1200
    uint16_t fscounts;  // 0x6000
    float fsrange;      // inst[1]: SCCM; inst[2]: PSIA; inst[3]: Celsius
    dns_aw_struct warn; // warning parameters
    dns_aw_struct alarm;// alarm parameters
    float offsetA;
    float gainA;
} dns_sas_object;
...
{
    uint8_t   devtype[8]; // 3
    uint8_t   semirev[16]; // 4
    uint8_t   manufacturer[16]; // 5
    uint8_t   model[8];   // 6
    uint8_t   fwrev[8];   // 7
    uint8_t   hwrev[8];   // 8
    uint8_t   sn[32];     // 9
    uint8_t   config[50]; // 10
    uint8_t   aEnable;    // 15
    uint8_t   wEnable;    // 16
} dns_sup_object;
...
{
    dns_datatype dType;      // INT: 0xC3; REAL: 0xCA
    dns_dataunits dUnits;    // Counts: 0x1001; Percent: 0x1007
    dns_aw_saa_struct warn; // warning parameters
    dns_aw_saa_struct alarm;// alarm parameters
} dns_saa_object;

typedef struct
{
    uint8_t eflag;     // enable flag
    float errBand;     // error band
    uint16_t SetTime;  // 0..65535
} dns_aw_scc_struct;
...
{
    dns_datatype dType;      // INT: 0xC3; REAL: 0xCA
    dns_dataunits dUnits;    // Counts: 0x1001; SCCM: 0x1400
    uint8_t cMode;
    dns_aw_scc_struct warn;
    dns_aw_scc_struct alarm;
} dns_scc_object;

typedef struct
{
    float     fsrange;  // 6 - 1
    dns_dataunits  dUnits;   // 6 - 2
} S_GAS_CALIBRATION_OBJECT_RANGE_T;

typedef struct
{
    float x;
    float y;
} cardinal_points_struct;

typedef struct
{
    uint8_t size;
    char lastcaldate[10];
    cardinal_points_struct cardinalPoints[MAX_CARDINAL_PTS];
} S_GAS_ONTOOL_CALIBRATION_OBJCET;

typedef struct
{
    uint16_t  gasNumber;
    uint16_t  sasInst;
    uint8_t   gasSymbol[128];
    uint8_t   gasName[128];
    S_GAS_CALIBRATION_OBJECT_RANGE_T     sFullScale;
    uint16_t   caldate;
    uint16_t  calGasNumber;
    float     calPressure;
    float     calTemperature;
    char factorycaldate[10];
    S_GAS_ONTOOL_CALIBRATION_OBJCET cardiPoints;

} dns_sgc_object;

// RTB must be multiples of 32 bytes
typedef struct
{
    uint32_t seqnum;  // 4
    double totalflow; // 8
...
// Auto Saves must be multiples of 32 bytes
typedef struct
{
    uint32_t seqnum;  // 4
    float azero;      // 4
...
typedef struct
{
    manufacturing_nvmem_struct manufacturing;
    network_struct network;
    //  dyn_controller dynGain;
    //  latched_error_struct error;
    //  analogcal_struct cal[4];
    analogcal_struct spcal[1];
    //  thermistor_cal_struct temperatureObj[2];
    nvpressure_struct pressureObj;
    nvflowmeter_struct meterObj;
    nvcontroller_struct controllerObj;
    //  nvgas_struct gasObj[6];
    //  dns_sup_object  supObj;
    //  dns_sas_object  sasObj[3];
    //  dns_saa_object  saaObj;
    //  dns_scc_object  sccObj;
    //  dns_sgc_object sgcObj[6];
} app_data;
...
extern uint8_t az_save_now;
extern uint32_t res_wr;
extern const char blfwrev[8];
extern nvmem_struct nvmem;
extern rtb_struct rtb;
extern az_struct autoz;
extern uint8_t is_flash_busy(void);
extern nvmem_state nvmem_stat_machine(void);
extern res_states restore_stat_machine(void);
extern uint8_t rtb_sm(void);
extern uint8_t az_sm(void);
extern void write_to_flash(uint16_t timeout);
extern uint32_t GetPage(uint32_t Addr);
extern uint32_t GetBank(uint32_t Addr);
extern void load_default_values(nvmem_struct *nv);
extern void clearNvMem(void);
