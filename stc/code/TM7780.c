...
u8	B_P_TestOneCycle_Mode;			//功率测量模式 1:单周期测量，0:1S定时测量
...
u8  	B_P_OVERFLOW;       			// 功率脉冲周期 溢出标志位
u8  	B_V_OVERFLOW;       			// 电压脉冲周期 溢出标志位
u8  	B_I_OVERFLOW;       			// 电流脉冲周期 溢出标志位

u8	B_P_Last_OVERFLOW;       		// 功率脉冲周期 溢出标志位
u8  	B_V_Last_OVERFLOW;       		// 电压脉冲周期 溢出标志位
u8  	B_I_Last_OVERFLOW;       		// 电流脉冲周期 溢出标志位

u8    	B_VI_Test_Mode;				//1:电压测量模式;0:电流测量模式
u16   	U16_VI_Test_Times;
u16   	U16_Cal_Times;

//u16   	power_usr.U16_AC_P;				//功率值 1000.0W
//u16   	power_usr.U16_AC_V;				//电压值 220.0V
//u16   	power_usr.U16_AC_I;				//电流值 4.545A
...
void TM7780_Measure_V(void)
{
    ...
    if (power_usr.U16_AC_V == 0xffff)     //开机时U32_V_CURRENT_PLUSEWIDTH_TIME = 0，计算溢出
    {
        power_usr.U16_AC_V = 0;
    }
...
void TM7780_Measure_I(void)
{
    ...
    if (power_usr.U16_AC_I == 0xffff)     //开机时U32_I_CURRENT_PLUSEWIDTH_TIME = 0，计算溢出
    {
        power_usr.U16_AC_I = 0;
    }
...
void tm7780_init(void)
{
    ...
    //鍒濆鍖朢OM
    Init_Rom();
    //璇诲彇瀛樻斁浜嶦EPROM鐨勬牎姝ｆ暟鎹?
    Read_CalData_EEPROM(); //璇诲彇鍙傝�冨姛鐜囥�佸弬鑰冪數鍘嬨�佸弬鑰冪數娴佺殑鑴夊啿鍛ㄦ湡
...
void tm7780_proc()
{
    ...
    else if (U8_CURR_WorkMode == D_CAL_END_MODE)	//鏍℃妯″紡
    {
        //璁板綍鏍℃鍊硷紝灏嗗綋鍓嶅姛鐜囥�佺數鍘嬨�佺數娴佸�煎瓨鍏EPROM;灏?.01搴︾數鐨勮剦鍐叉暟鍙傚�间篃瀛樺叆EEPROM
