	...
	else
	{
		switch(csyi)
		{
			...
			case 2:
			{
				...
				{
					pt_disp(1);
					if(da_tt)swzs_disp(2,voltage_vary_ratio);
					else bxs_disp(2);
				}
				else 
				if(cser==5)		//电流变比
				{
					ct_disp(1);
					if(da_tt)swzs_disp(2,current_vary_ratio);
					else bxs_disp(2);
				}
			}break;
			case 3:
...
void led_disp(void)
{
	...
	{
//		date=ax_av*voltage_vary_ratio;
//		ywxs_disp(0,date);	//电压A
//		date=bx_av*voltage_vary_ratio;
//		ywxs_disp(1,date);	//电压B
//		date=cx_av*voltage_vary_ratio;
//		ywxs_disp(0,date);	//电压C
		if(xian==0)
		{
			date=ax_aa*current_vary_ratio;
			swxs_disp(0,date);	//电流A
			date=bx_aa*current_vary_ratio;
			swxs_disp(1,date);	//电流B
			date=cx_aa*current_vary_ratio;
