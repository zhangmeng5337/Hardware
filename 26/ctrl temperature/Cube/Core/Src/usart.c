...
#include "string.h"
#include "app_fifo.h"
//uart1 pc通信 波特率：115200，奇偶校验：无，数据位：8，停止位：1
//uart2  mfc通信 波特率：9600，奇偶校验：无，数据位：8，停止位：2
//uart3 六通阀通信 波特率：9600，奇偶校验：无，数据位：8，停止位：1
//uart4 十二通阀通信 波特率：9600，奇偶校验：无，数据位：8，停止位：1
...
uint8_t rx_buffer2[BUFFER_SIZE1]={0};  //接收数据缓存数组
/////////串口3
volatile uint8_t rx_len3 = 0;  //接收一帧数据的长度
volatile uint8_t recv_end_flag3 = 0; //一帧数据接收完成标志
uint8_t rx_buffer3[BUFFER_SIZE]={0};  //接收数据缓存数组
//////////////
/////////串口4
volatile uint8_t rx_len4 = 0;  //接收一帧数据的长度
volatile uint8_t recv_end_flag4 = 0; //一帧数据接收完成标志
uint8_t rx_buffer4[BUFFER_SIZE]={0};  //接收数据缓存数组
//////////////
/////////串口5
volatile uint8_t rx_len5 = 0;  //接收一帧数据的长度
volatile uint8_t recv_end_flag5 = 0; //一帧数据接收完成标志
uint8_t rx_buffer5[BUFFER_SIZE]={0};  //接收数据缓存数组
/////////串口6 
volatile uint8_t rx_len6 = 0;  //接收一帧数据的长度
volatile uint8_t recv_end_flag6 = 0; //一帧数据接收完成标志
uint8_t rx_buffer6[BUFFER_SIZE]={0};  //接收数据缓存数组



#define RXBUFFERSIZE  256     //最大接收字节数
//char RS485_RX_BUFF[512]  ; //接收数据
//char Receive_Data[RXBUFFERSIZE];
//int receive_flag; 
...
void MX_UART4_Init(void)
{
  ...
  /* USER CODE BEGIN UART4_Init 2 */
	...
	HAL_UART_Receive_DMA(&huart4,rx_buffer4,BUFFER_SIZE);
...
void MX_UART5_Init(void)
{
  ...
  /* USER CODE BEGIN UART5_Init 2 */
	...
	HAL_UART_Receive_DMA(&huart5,rx_buffer5,BUFFER_SIZE);
...
void MX_USART2_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART2_Init 2 */
	...
	HAL_UART_Receive_DMA(&huart2,rx_buffer2,BUFFER_SIZE);
...
void MX_USART6_UART_Init(void)
{
  ...
  /* USER CODE BEGIN USART6_Init 2 */
	...
	HAL_UART_Receive_DMA(&huart6,rx_buffer6,BUFFER_SIZE);
