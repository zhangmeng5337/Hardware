...
Module_Params_stru  Module_Params1;
...
void ModuleInit()
{
  ...
  Module_Params1.CheckBit = 0;  
  Module_Params1.ADDH = 0x01;
  Module_Params1.ADDM = 0x04;

  Module_Params1.ADDL = 0x03;
  
  Module_Params1.Flash_Write_Done = 1;  
  Module_Params1.AirRate = 2;
  Module_Params1.Channel =0x17;
  Module_Params1.SerialRate = 0x07;
  Module_Params1.WakeupTime = 250;
...
void RF_Initial( )
{
  ...
  if(FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 9)) == 1)  //判断Flash中是否有数据
  {
    ...
    Module_Params1.Channel = FLASH_ReadByte(FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS);
    Module_Params1.AirRate = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 1));
    Module_Params1.power = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 2));
    Module_Params1.SerialRate = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 3));
    Module_Params1.CheckBit = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 4));
    Module_Params1.WakeupTime = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 5));	
    Module_Params1.ADDH = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 6));	
    Module_Params1.ADDL = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 7));	
    Module_Params1.TranMode = FLASH_ReadByte((FLASH_DATA_EEPROM_START_PHYSICAL_ADDRESS + 8));	
  } 
  else
    ModuleInit();
  
  SX1276_Init(MODE_LORA);         //SX127X 模块复位并初始化为LoRa模式
  switch( Module_Params1.AirRate)
  {
  case 0x00:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//0.3kbps,
  case 0x01:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//1.2kbps
  case 0x02:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//2.4kbps
  case 0x03:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//4.8kbps
  case 0x04:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//9.6kbps
  case 0x05:  SX1276_LoRa_SetDataRate(Module_Params1.AirRate);break;//19.2kbps
  default:  SX1276_LoRa_SetDataRate(0x03);break;//4.8kbps
  }
  
  if(Module_Params1.Channel<32)
    SX1276_SetFreq(Module_Params1.Channel);				//配置信道为23，即433Hz
  else
    SX1276_SetFreq(23);				//配置信道为23，即433Hz 
  if( Module_Params1.power*2<=14&&Module_Params1.power>0)
  {
    SX1276_SetPower(Module_Params1.power*2, 1);		  //配置RFO PIN输出，功率20dBm
  }
  else
