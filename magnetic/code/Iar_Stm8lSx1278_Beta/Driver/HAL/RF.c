...
{//uint32_t tx_cnt=0;uint32_t count_recv;
  ...
  if(Module_mode.ConfigureDone) 
  {
    ...
    if( ExitInterFlag) 
    {
      ...
      {
        if(xorCheck(&recv_buffer[2],length-2)^recv_buffer[7]==recv_buffer[7]||xorCheck(&recv_buffer[2],length-2)^recv_buffer[8]==recv_buffer[8])
...
>>>>>>> 1de386f8b5859101818602515ef230e12da9abaf
