...
void NodeToLORA()
{
	...
	uTmpLen = uTmpLen+6;
	memcpy(pbuffer,wrcommand ,DataPack.server_channelH);
	uTmpLen = uTmpLen+1;
	memcpy(pbuffer,wrcommand ,DataPack.server_channelL);
	uTmpLen = uTmpLen+1;
	memcpy(pbuffer,wrcommand ,DataPack.serverAirRate);
	uTmpLen = uTmpLen+1;
	...
	switch(uartparase(2,1))
	{
		...
		case SET_PARAMS:
		{
            ...
            if(uart2.receive_buffer[6]>=6)
            {
				...
				DataPack.server_channelH=uart2.receive_buffer[12];
				DataPack.server_channelL=uart2.receive_buffer[13];
				DataPack.serverAirRate=uart2.receive_buffer[14];
			...
			case SET_PERIOD:
			{
				...
				if(uart2.receive_buffer[6]>=6)
				{
					...
					//DataPack.id[1] = uart2.receive_buffer[3];;
					//DataPack.id[2] = uart2.receive_buffer[4];;
					DataPack.tx_period = uart2.receive_buffer[12];
					HAL_NVIC_SystemReset();
				}
			}break;
			...
			case SET_THRES:
			{
				...
				if(uart2.receive_buffer[6]>=6)
				{
					 ...
					 MIN_THRES==uart2.receive_buffer[12];
					 MAX_THRES==uart2.receive_buffer[13];
