   ...
   //根据不同的菜单显示层级对按键进行处理，并显示恰当的LED值
    switch(Menu_ID){
        case 1:        
           ...
           if(Key2_status==1){
                Fun_Para[1]++;
                if(Fun_Para[1]>MAXSET_THERM)Fun_Para[1]=MAXSET_THERM;
                Menu_ID_DS=3;
                RestDsCount=0;
                Update_Disp=1;
                Clean_KeyV();
           ...
           if(Key3_status==1){
                Fun_Para[1]--;
                if(Fun_Para[1]<MINSET_THERM)Fun_Para[1]=MINSET_THERM;
                Menu_ID_DS=3;
                RestDsCount=0;
                Update_Disp=1;
                Clean_KeyV();
...
void CoolWaterPIDCompute(){
 ...
 if (Timers.PID_timer>100){
           ...
           EK2 = EK1 ;
           EK1 = EK ;
           EK = AD_Value[2]-Fun_Para[1]*10;  

           switch(PIDState){
                   case 0:         
                         if(EK<0)PIDState=1;  //如果目标1度范围类这进入状态1    
                         if(Timers.PIDState0_timer>36000)PIDState=1;  //如果在6分钟后     
