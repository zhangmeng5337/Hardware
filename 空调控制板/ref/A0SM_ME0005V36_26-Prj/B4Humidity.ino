   ...
   //根据不同的菜单显示层级对按键进行处理，并显示恰当的LED值
    switch(Menu_ID){
        case 1:        
           ...
           if(Key2_status==1){
                Fun_Para[5]++;
                if(Fun_Para[5]>MAXSET_HUMIDITY)Fun_Para[5]=MAXSET_HUMIDITY;
                Menu_ID_DS=3;
                RestDsCount=0;
                Update_Disp=1;
                ParaSaveFlag=1;
           ...
           if(Key3_status==1){
                Fun_Para[5]--;
                if(Fun_Para[5]<MINSET_HUMIDITY)Fun_Para[5]=MINSET_HUMIDITY;
                Menu_ID_DS=3;
                RestDsCount=0;
                Update_Disp=1;
                ParaSaveFlag=1;
            ...
            if(Error==0){
                   ...
                   switch(WorkState){
                        case 0:
                                 ...
                                 if(DelayCountDown<0){
                                    Diff=AD_Value[0]-Fun_Para[5]*10;  //检测温度与设定温度差值
                                    if(Diff<5){
                                         WorkState=5;          
                                    }else{
                                        WorkState=1; 
...
void HumidityPIDCompute(){
 ...
 if (Timers.PID_timer>100){
           ...
           EK2 = EK1 ;
           EK1 = EK ;
           EK = AD_Value[0]-Fun_Para[5]*10;    //湿度差异
